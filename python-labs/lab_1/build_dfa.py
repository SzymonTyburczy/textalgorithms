...
def build_dfa(regex: RegEx, alphabet: set[str]) -> Optional[DFA]:
    ...
    # Uprość początkowe wyrażenie i utwórz pierwszy stan
    initial_regex = simplify(regex)
    start_state = f"q{state_counter}"
    state_counter += 1

    states.add(start_state)
    state_to_regex[start_state] = initial_regex
    regex_to_state[str(initial_regex)] = start_state
    if initial_regex.nullable():
        accept_states.add(start_state)

    queue = deque([start_state])

    # Przetwarzaj stany - dla każdego symbolu oblicz pochodną i utwórz nowe stany
    while queue:
        current_state = queue.popleft()
        current_regex = state_to_regex[current_state]

        for symbol in alphabet:
            # Oblicz pochodną i uprość wyrażenie
            derived = simplify(current_regex.derivative(symbol))
            derived_str = str(derived)

            # Sprawdź, czy już mamy stan reprezentujący to wyrażenie
            if derived_str in regex_to_state:
                next_state = regex_to_state[derived_str]
            else:
                next_state = f"q{state_counter}"
                state_counter += 1
                regex_to_state[derived_str] = next_state
                state_to_regex[next_state] = derived
